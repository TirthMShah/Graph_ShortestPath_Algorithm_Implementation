# Find's the path for undirected graph using BFS.
...
def add_random_neighbors(nodes, probability,G):
    ...
    for node in nodes:
        ...
        for other_node in other_nodes:
            ...
            if node != other_node and count <= 1 :
                ...
                G.add_edge(other_node.name, node.name, weight=1) 
            count+=1
...
def main():
    ...
    nodes = create_nodes([i for i in range(50)], G)
    ...
    nx.draw_networkx_edges(G, pos, alpha=0.5) #For undirected graph.
